...
const BillingTabComponent = (props: Props) => {
  ...
  return (
    <Box>
        ...
        <Typography sx={BillingTabStyles.tableHeadingFilterText}>Filter</Typography>
      </Box>
    <Box>
      <TableContainer sx={BillingTabStyles.tableContainer} component={Paper}>
        <Table>
          <TableHead>
            <TableRow>
              {billingData[0] &&
                Object.entries(billingData[0]).map(
                  ([key, value], index) =>
                    key !== "id" && <TableCell sx={BillingTabStyles.tableHead} key={index}>
                      <Box sx={BillingTabStyles.tableHeadCellContainer}>
                      <Typography sx={BillingTabStyles.tableHeadText}>{value}</Typography>
          ...
          <TableBody>
            {billingData.slice(1).map((row) => (
              <TableRow key={row.id}>
                <TableCell sx={BillingTabStyles.tableBodyCells}>{row.billingDate}</TableCell>
